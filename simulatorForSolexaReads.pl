...
my $rawreadsfolder = tempdir( $rawreadsname, DIR => $outfolder);
#my $rawreads = tempdir ( $rawreadsname, DIR => $outfolder, CLEANUP => 1);

open(GENOMEINFO, "<$genomeInfo") or die "Cannot open the genome information file: $abundanceFile\n";
...
for $currentGenome (keys %abundanceHash ) {
	$newAbundanceHash{$currentGenome} = round($abundanceHash{$currentGenome}*$multiplicator);
}
...
while (my $fasta_2_obj_entry  = $fasta_2_obj->next_seq){
  ...
  die "Fasta and Quality file are not synchronized!\n" unless $fasta_2_obj_entry->id eq $qual_2_obj_entry->id;
