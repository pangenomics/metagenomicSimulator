...
my $leftStartOnContig = 0;
my $rightstartOnContig = 0;
...
my $readLocations = "$outfolder\/readLocations.txt";
open(READLOC, ">$readLocations") or die "Cannot open the data file for data output: $readLocations\n";
...
for my $currentGenome (keys %readAbundanceHash ) {
	...
	open PIPE, "$binfolder\/generateReads_d -i $genomeFile -l $insertSize -c $insertnumber -s $insertSD |" or die "Cannot open pipe from $binfolder\/generateReads: $!";
	...
	while ( my $currentFastaLine=<PIPE> ){
         ...
         if (substr($currentFastaLine,0,1) eq "\>") {
			if ($currentInsertName ne ""){
				...
				$currentInsertName =~ m/>(.*)__(\d+):(\d+):.*/;
				my $contig = $1;
				my $start = $2;
				my $end = $3;
				
				my $leftReadStart = $start + $leftStartOnContig + 1;
				my $leftReadEnd = $start + $leftStartOnContig + $readLength;
				my $rightReadStart = $start + $rightstartOnContig + 1;
				my $rightReadEnd = $start + $rightstartOnContig + $readLength;
				
				print READLOC "$leftReadName\t$currentGenome\t$contig\t$leftReadStart\t$leftReadEnd\n";
				print READLOC "$rightReadName\t$currentGenome\t$contig\t$rightReadStart\t$rightReadEnd\n";
			}
...
$fasta_1_obj_entry->id eq $qual_1_obj_entry->id;
...
sub getReadsFromInsert{
     ...
     $rightEnd = substr($insertSequence,length($insertSequence)-$readLength,$readLength);
     ...
     if ($strand == 0) {
		 ...
		 $leftStartOnContig = 0;
		 $rightstartOnContig = length($insertSequence)-$readLength;
		 ...
		 $leftStartOnContig = length($insertSequence)-$readLength;
		 $rightstartOnContig = 0; 
